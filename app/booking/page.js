...
export default function BookingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Email is required when saving card
    ...
    try {
      // Step 1: submit booking form as usual
      ...
      formData.append("paymentMethod", paymentMethod === "card" ? "Card (Stripe)" : "Cash / Pay after service");
      ...
      // Step 2: if card selected, redirect to Stripe to save card
      if (paymentMethod === "card") {
        ...
        // If Stripe redirect fails, still show success (booking was saved)
      }
      ...
      <section className="max-w-7xl mx-auto px-5 pb-24">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-[40px] p-6 md:p-8 grid gap-4 shadow-2xl border border-black/10">
            ...
            {/* Email — highlighted when card is selected */}
            <div>
              <input
                name="email"
                type="email"
              ...
              {status === "email-required" && (
                <p className="text-orange-600 text-xs font-black mt-1">Email is required to save your card.</p>
              )}
            </div>
            ...
            <div>
              ...
              {paymentMethod === "card" && (
                <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
                  Your card is saved securely via Stripe — no charge now. We'll charge once the job is confirmed and completed.
                </p>
              )}
            </div>
